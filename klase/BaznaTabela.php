...
public function IzvrsiAktivanSQLUpit($AktivanSQLUpit) {
	// PRIMER: INSERT / UPDATE / DELETE naredba nad tabelama ARTIKAL, PORUDZBINA ili STAVKA_PORUDZBINE.
    // ---------------
	if ($this->TipMYSQL=="mysqli")
	{
...
// *************************************************************
public function EscapirajVrednost($Vrednost)
{
	if ($Vrednost === null)
	{
		return '';
	...
	if ($this->TipMYSQL=="mysqli")
	{
		return mysqli_real_escape_string($this->OtvorenaKonekcija->konekcijaDB, (string) $Vrednost);
	}

	return mysql_real_escape_string((string) $Vrednost);
}
// *************************************************************
public function VratiSveAsocijativno($Upit)
{
	$Rezultat = array();

	if ($this->TipMYSQL=="mysqli")
	{
		$Kolekcija = mysqli_query($this->OtvorenaKonekcija->konekcijaDB, $Upit);
		if ($Kolekcija)
		{
	...
	{
		$Kolekcija = mysql_query($Upit);
		if ($Kolekcija)
		{
			while ($RedZapisa = mysql_fetch_assoc($Kolekcija))
			{
				$Rezultat[] = $RedZapisa;
			}
		}
...
// *************************************************************
public function VratiPrviAsocijativniRed($Upit)
{
	$Redovi = $this->VratiSveAsocijativno($Upit);
	if (count($Redovi) > 0)
	{
		return $Redovi[0];
	}
...
// *************************************************************
public function DajBrojPogodjenihRedova()
{
	if ($this->TipMYSQL=="mysqli")
	{
		return mysqli_affected_rows($this->OtvorenaKonekcija->konekcijaDB);
	}

	return mysql_affected_rows($this->OtvorenaKonekcija->konekcijaMYSQL);
}
// *************************************************************
public function DajPoslednjiInsertId()
{
	if ($this->TipMYSQL=="mysqli")
	{
	...
	return mysql_insert_id($this->OtvorenaKonekcija->konekcijaMYSQL);
}
